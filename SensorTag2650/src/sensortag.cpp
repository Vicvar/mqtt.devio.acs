...
{
  std::cout << "getTemperature()"<< std::endl;
  return temperature;
}

double getLight ()
{
  std::cout << "getLight()"<< std::endl;;
  return light;
}

double getHumidity ()
{
  std::cout << "getHumidity()"<< std::endl;
  return humidity;
}
...
int main(int argc, char* argv[])
{
  ...
  args[2] = const_cast<char*>(STMAC);
  args[3] = const_cast<char*>("-I");
  
  while (1)
    {
      std::cout << "Connecting to device: [" << STMAC << "]" << std::endl;
