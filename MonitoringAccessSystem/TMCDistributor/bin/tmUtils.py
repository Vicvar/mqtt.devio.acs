...
#
# "@(#) $Id: tmUtils.py,v 1.0 2015/12/04 08:00:00 pmerino Exp $"
#
# who       when      what
# --------  --------  ----------------------------------------------
...
def get_tmc_data(antenna, device, monitorpoint, startdate, enddate, \
    ...
    if len(files) == 0:
        print "Failed to retrieve data files."
	return None
    else:
	datetimelist = []
	valuelist = []
...
def check_for_time_gaps(antenna='DV01', device='IFProc0', monitorpoint='GAINS',
    ...
    if (tmcdata!=None):
      ...
      lost_data_percent = sum(gaps)*100.0/(unixtime[-1]-unixtime[0]+medianInterval)
      if (len(gaps) > 0):
          print "Found %d gaps of median size = %f seconds" % (len(gaps), np.median(gaps))
          print "Lost data = %f seconds = %f percent of total dataset" % (sum(gaps), lost_data_percent)
      return gaps, lost_data_percent
    else:
      return None, None
